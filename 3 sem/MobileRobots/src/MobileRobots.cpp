...
namespace MobileRobots
{
    ...
    void MobileRobots::updateInfo(const Coord& coord /* = { -1, -1 } */)
    {
        QString text;
        ...
        m_ui->infoWidget->appendHtml(text);
        m_ui->infoWidget->moveCursor(QTextCursor::Start);
    }

    void MobileRobots::showEvent(QShowEvent* event)
    {
        draw();

        const auto WIDTH = m_ui->canvas->width();
        const auto HEIGHT = m_ui->canvas->height();

        m_scene->setSceneRect(0, 0, WIDTH, HEIGHT);
        m_ui->canvas->setScene(m_scene.get());
        for (auto&& row : m_map)
            for (auto&& obj : row)
                m_scene->addItem(obj);
    ...
    void MobileRobots::mousePressEvent(QMouseEvent* event)
    {
        if (event->x() <= m_ui->canvas->width() && event->y() <= m_ui->canvas->height())
            updateInfo({ event->x() / m_scaleFactor, event->y() / m_scaleFactor });
        ...
        m_scaleFactor(MobileRobots::IMAGE_SIZE)
    {
        ...
        m_ui->canvas->resize(SCALED_CANVAS_WIDTH, SCALED_HEIGHT);
    ...
    void MobileRobots::draw() const
    {
        for (auto&& row : m_map)
            for (auto obj : row)
                obj->setPixmap(m_images.at("DarkGrass"));
